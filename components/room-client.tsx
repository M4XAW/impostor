...
const settingFields: Array<{ name: SettingName; label: string; min: number; max: number }> = [
...
function isGameSnapshot(value: unknown): value is GameSnapshot {
  return typeof value === "object" && value !== null && "phase" in value && "players" in value && "settings" in value;
}
...
export function RoomClient({ code }: RoomClientProps) {
  ...
  async function play(payload: Record<string, unknown>) {
    const response = await fetch(`/api/rooms/${code}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    const result: unknown = await response.json().catch(() => null);
    if (!response.ok) setError(typeof result === "object" && result !== null && "error" in result && typeof result.error === "string" ? result.error : "Action impossible.");
    await refresh();
  }
  ...
  if (error && !game) return <main className="grid min-h-screen place-items-center bg-[#12151f] p-6 text-center text-white"><div><p>{error}</p><Link className="mt-4 inline-block text-orange-300 underline" href="/">Retour à l’accueil</Link></div></main>;
  if (!game) return <main className="grid min-h-screen place-items-center bg-[#12151f] text-slate-300">Chargement…</main>;
  ...
  {game.phase === "LOBBY" && <div className="mt-6 space-y-4"><div className="rounded-2xl bg-white/5 p-4"><p className="text-sm text-slate-400">Lien d’invitation</p><div className="mt-2 flex gap-2"><code className="min-w-0 flex-1 truncate text-sm">?join={game.code}</code><button className="rounded-lg bg-violet-500 px-3 py-1.5 text-sm font-bold" onClick={() => void copyInvite()}>{isCopied ? "Copié" : "Copier"}</button></div></div><Settings game={game} isHost={isHost} onSave={play} /></div>}
  ...
  {game.phase === "VOTING" && <ul className="mt-7 space-y-2">{game.players.map((player) => <li key={player.id} className="flex items-center gap-3 rounded-xl border border-white/10 px-4 py-3"><span className="min-w-0 flex-1 truncate">{player.name}{player.id === game.currentPlayer.id ? " (toi)" : ""}</span><span className="text-sm text-slate-400">{player.hasVoted ? "A voté" : ""}</span>{player.id !== game.currentPlayer.id && <button className="rounded-lg bg-white/10 px-3 py-1 text-sm hover:bg-white/20" onClick={() => void play({ action: "vote", targetId: player.id })}>Voter</button>}</li>)}</ul>}

  {game.phase === "VOTING" && <div className="mt-6"><h2 className="font-bold">Votes visibles</h2>{game.votes.length === 0 ? <p className="mt-2 text-sm text-slate-400">Aucun vote pour le moment.</p> : <ul className="mt-2 space-y-2">{game.votes.map((vote) => <li key={`${vote.voterName}-${vote.targetName}`} className="rounded-xl bg-white/5 px-4 py-2 text-sm"><strong>{vote.voterName}</strong> a voté pour <strong>{vote.targetName}</strong></li>)}</ul>}</div>}

  {error && <p className="mt-4 text-sm text-rose-300" role="alert">{error}</p>}
  <div className="mt-7">{game.phase === "LOBBY" && isHost && <button disabled={game.players.length < 3} className="rounded-xl bg-orange-500 px-5 py-3 font-bold disabled:opacity-40" onClick={() => void play({ action: "start" })}>Lancer la partie</button>}{game.phase === "DISCUSSION" && isHost && game.turn?.canStartVote && <button className="rounded-xl bg-violet-500 px-5 py-3 font-bold" onClick={() => void play({ action: "beginVote" })}>Passer au vote</button>}</div></section></div></main>;
}

function Settings({ game, isHost, onSave }: { game: GameSnapshot; isHost: boolean; onSave: (payload: Record<string, unknown>) => Promise<void>; }) {
  if (!isHost) return <div className="rounded-2xl border border-white/10 p-4 text-sm text-slate-300"><p className="font-bold text-white">Paramètres de l’hôte</p><p className="mt-2">{game.settings.wordCount} mots · {game.settings.roundCount} tours · {game.settings.turnSeconds}s par tour · {game.settings.impostorCount} imposteur{game.settings.impostorCount > 1 ? "s" : ""}</p></div>;
  return <form key={`${game.settings.wordCount}-${game.settings.roundCount}-${game.settings.turnSeconds}-${game.settings.impostorCount}`} className="rounded-2xl border border-white/10 p-4" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void onSave({ action: "settings", wordCount: Number(data.get("wordCount")), roundCount: Number(data.get("roundCount")), turnSeconds: Number(data.get("turnSeconds")), impostorCount: Number(data.get("impostorCount")) }); }}><h2 className="font-bold">Paramètres de partie</h2><div className="mt-3 grid grid-cols-2 gap-3 text-sm">{settingFields.map((field) => <label key={field.name} className="text-slate-300">{field.label}<input name={field.name} type="number" min={field.min} max={field.max} defaultValue={game.settings[field.name]} className="mt-1 w-full rounded-lg bg-white/10 px-3 py-2 text-white"/></label>)}</div><button className="mt-4 rounded-lg bg-white/10 px-3 py-2 text-sm font-bold hover:bg-white/20">Enregistrer</button></form>;
}
