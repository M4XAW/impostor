...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!createRequest.success) return Response.json({ error: "Informations de partie invalides." }, { status: 400 });
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Erreur de partie." }, { status: 400 });
  }
}
